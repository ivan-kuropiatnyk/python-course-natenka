...
class CiscoTelnet:
    def __init__(self, parameters):
        ...
        self.telnet.write(to_bytes(self.username))
        self.telnet.read_until(b"Password")
        self.telnet.write(to_bytes(self.password))
        index, m, output = self.telnet.expect([b">", b"#"])
        if index == 0:
            self.telnet.write(b"enable\n")
            self.telnet.read_until(b"Password")
            self.telnet.write(to_bytes(self.secret))
    ...
    def send_show_command(self, commands):
        ...
        for command in commands:
            self.telnet.write(to_bytes(command))
