...
def parse_sh_version(sh_ver_output):
    ...
    match = re.search(regex, sh_ver_output, re.DOTALL,)#re.DOTALL - превратил точку в любой символ включая \n
    if match:
...
def write_inventory_to_csv(data_filenames, csv_filename):
    ...
    with open(csv_filename, "w") as f:#открытие файла на запись и внутри этого файла будет открываться файлы на чтение, после обработки которых результат запишется в этот файл
        writer = csv.writer(f)#приравнивание переменной к открытому файлу формата .csv
        writer.writerow(headers)#запись в файл строки с headers

        for filename in data_filenames:#перебор заданных файлов
            hostname = re.search("sh_version_(\S+).txt", filename).group(1)#поиск в имени файла
            with open(filename) as f:#
                parsed_data = parse_sh_version(f.read())#f.read() - считывает файл в одну строку
                if parsed_data:
...
if __name__ == "__main__":
    ...
    write_inventory_to_csv(sh_version_files, "page417_routers_inventory.csv")